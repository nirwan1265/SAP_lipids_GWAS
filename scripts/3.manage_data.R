################################################################################
############# Loading the Filtered and Normalized Data from 2.  ################
################################################################################

# Loading datasets
A <- read.csv("results/SERRF/A_SERRF.csv")
A <- A[-c(1:2),-1]
B <- read.csv("results/SERRF/B_SERRF.csv")
B <- B[-c(1:2),-1]
#B_trial <- read.csv("data/SetB_lipid_FLO2022_lowP.csv")
...
# Convert PI columns to numeric
subset_A_PC_unique <- subset_A_PC_unique %>%
  mutate(across(starts_with("PI"), ~as.numeric(as.character(.))))
subset_B_PC_unique <- subset_B_PC_unique %>%
  mutate(across(starts_with("PI"), ~as.numeric(as.character(.))))
...
# Saving the file
#write.csv(PC_A,"results/PC/PC_A.csv",row.names=F)
#write.csv(PC_B,"results/PC/PC_B.csv",row.names=F)


# Have to add all the bonds to 
# Eg: PC(18:0/18:2) = PC(36:2)
# Did this in excel using data to column by sep
# deleted Spectral Match to Arachidonoylthio-PC from NIST14 for B_PC.csv and A_PC.csv
# Now need to sum up the common lipids
A_PC_filtered <- read.csv("results/PC/PC_A_combined.csv")
B_PC_filtered <- read.csv("results/PC/PC_B_combined.csv")

A_PC_filtered[1:10,1:5]


# Group by 'Compound_Prefix' and then sum all other columns
A_PC_filtered_sum <- A_PC_filtered %>%
  dplyr::group_by(Compound_Prefix) %>%
  dplyr::summarize(across(everything(), sum))

B_PC_filtered_sum <- B_PC_filtered %>%
  dplyr::group_by(Compound_Prefix) %>%
  dplyr::summarize(across(everything(), sum))

# Remove the odd numbered lipids cause plants dont have them like 17:0
A_PC_filtered_sum <- A_PC_filtered_sum[-c(1,10,11,12), ]
B_PC_filtered_sum <- B_PC_filtered_sum[-c(1,7,8), ]

A_PC_filtered_sum

# Transposing the df for GWAS
A_PC_filtered_sum_transposed <- A_PC_filtered_sum %>%
  dplyr::select(-Compound_Prefix) %>%
  t() %>%
  as.data.frame()

B_PC_filtered_sum_transposed <- B_PC_filtered_sum %>%
  dplyr::select(-Compound_Prefix) %>%
  t() %>%
  as.data.frame()

# Add the Compound_Prefix as column names
colnames(A_PC_filtered_sum_transposed) <- A_PC_filtered_sum$Compound_Prefix
colnames(B_PC_filtered_sum_transposed) <- B_PC_filtered_sum$Compound_Prefix

# Convert row names to a column (they should be the original PI identifiers)
A_PC_filtered_sum_transposed <- rownames_to_column(A_PC_filtered_sum_transposed, var = "PI_ID")
B_PC_filtered_sum_transposed <- rownames_to_column(B_PC_filtered_sum_transposed, var = "PI_ID")

A_PC_filtered_sum_transposed[1:5,1:5]


# Log transform and median-center the data
A_PC_filtered_sum_transposed <- A_PC_filtered_sum_transposed %>%
  dplyr::mutate(across(-PI_ID, ~log(.) - median(log(.), na.rm = TRUE)))
B_PC_filtered_sum_transposed <- B_PC_filtered_sum_transposed %>%
  dplyr::mutate(across(-PI_ID, ~log(.) - median(log(.), na.rm = TRUE)))


hist(B_PC_filtered_sum_transposed$`PC(34:3)`)

# Saving for GWAS
write.csv(A_PC_filtered_sum_transposed,"results/GWAS_pheno/A_PC_pheno.csv",row.names=F,quote=F)
write.csv(B_PC_filtered_sum_transposed,"results/GWAS_pheno/B_PC_pheno.csv",row.names=F,quote=F)
